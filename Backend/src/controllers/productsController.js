...
export const obtenerProductos = async (req, res) => {
  try {
    ...
    const [estados] = await conn.execute(`SELECT Id_Estado, Descripcion FROM EstadoProducto`);
    conn.release();
    ...
    for (const estado of estados) {
      mapaEstados[estado.Id_Estado] = estado.Descripcion;
    }

    const mapear = (arr) => arr.map(p => ({ 
      ...p, 
      BaseDatos: mapaBases[p.Id_Base] || 'Desconocido',
      EstadoDescripcion: mapaEstados[p.Activo] || (p.Activo === 1 ? 'Activo' : 'Inactivo')
    }));
...
// Implementar en productsController.js
export const cambiarEstadoProducto = async (req, res) => {
  try {
    const { CodigoProducto, Activo, Id_Base, todasLasBases } = req.body;
    ...
    // Valor de Activo opuesto al recibido (para activar/desactivar)
    const nuevoEstado = Activo === 1 ? 2 : 1;
    ...
    const tryUpdate = async (baseId, label) => {
      try {
        switch (baseId) {
          case 2: // MySQL
            const connMySQL = await poolMySQL.getConnection();
            const [resultMySQL] = await connMySQL.execute(
              'UPDATE Products SET Activo = ? WHERE CodigoProducto = ?',
              [nuevoEstado, CodigoProducto]
            );
            connMySQL.release();
            if (resultMySQL.affectedRows > 0) {
              resultados.exitosos.push(label);
            }
            break;
          ...
          case 3: // SQL Server
            await poolSQLServer.connect();
            const resultSQLS = await poolSQLServer.request()
              .input("Activo", nuevoEstado)
              .input("CodigoProducto", CodigoProducto)
              .query('UPDATE Products SET Activo = @Activo WHERE CodigoProducto = @CodigoProducto');
            if (resultSQLS.rowsAffected[0] > 0) {
              resultados.exitosos.push(label);
            }
            break;
            
          case 4: // PostgreSQL
            const clientPG = await poolPostgres.connect();
            const resultPG = await clientPG.query(
              'UPDATE Products SET activo = $1 WHERE codigoproducto = $2',
              [nuevoEstado, CodigoProducto]
            );
            clientPG.release();
          ...
          case 5: // Oracle
            const connOracle = await poolOracle.getConnection();
            const resultOracle = await connOracle.execute(
              'UPDATE Products SET Activo = :1 WHERE CodigoProducto = :2',
              [nuevoEstado, CodigoProducto],
              { autoCommit: true }
            );
            await connOracle.close();
            if (resultOracle.rowsAffected > 0) {
              resultados.exitosos.push(label);
            }
            break;
        }
        ...
        resultados.errores.push(`${label}: ${error.message}`);
      }
    };
    
    // Determinar en qué bases actualizar
    const bases = todasLasBases ? [2, 3, 4, 5] : [Id_Base];
    ...
    return res.status(200).json({
      exito: true,
      mensaje: `Producto ${accion} correctamente`,
      basesExito: resultados.exitosos,
      basesError: resultados.errores,
      nuevoEstado
    });
